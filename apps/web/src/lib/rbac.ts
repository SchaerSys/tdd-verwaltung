 ...
 */
export type Role = "ADMIN" | "ERFASSUNG" | "AUSGABE" | "AUSWERTUNG" | "SACHBEARBEITER" | "FAHRER" | "MITARBEITER";
...
export type Permission =
  ...
  | "self:view";        // P1 Mein Bereich: eigene Zeiten/Urlaubskonto, Urlaubsantrag, Krankmeldung

const MATRIX: Record<Role, Permission[]> = {
  ADMIN: ["person:read", "person:write", "card:manage", "distribution:record", "report:view", "admin:manage", "antrag:manage", "document:view", "staff:manage", "tour:manage", "tour:drive", "self:view"],
  ERFASSUNG: ["person:read", "person:write", "card:manage", "distribution:record", "tour:manage", "self:view"], // KEINE Dokumente; Buero disponiert Touren
  AUSGABE: ["distribution:record"], // nur Scan + Ausgabe, KEINE Personenlisten/Dokumente
  AUSWERTUNG: ["report:view", "self:view"],
  MITARBEITER: ["self:view"], // nur der eigene Bereich (Zeiten, Urlaub) – keine Klientendaten
