...
export async function einrichtungOffen(): Promise<boolean> {
  const r = await (await db()).execute(sql`SELECT count(*)::int AS n FROM ops_users`);
...
export async function ersteEinrichtung(_prev: SetupState, fd: FormData): Promise<SetupState> {
  ...
  await (await db()).insert(opsUsers).values({ email, displayName: name, passwordHash: await hash(pw) });
