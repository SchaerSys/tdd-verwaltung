...
export function checkliste(m: MandantStatus): Punkt[] {
  return [
    { key: "host", titel: "Eigener Host zugeordnet", ok: !!m.host, hinweis: m.host ? m.host : "Ohne Host melden sich Benutzer über ihre E-Mail-Adresse an; die Fach-App zeigt dann zunächst den Standard-Mandanten (Login-Seite).", href: `/unternehmen/${m.id}#stammdaten` },
    { key: "admin", titel: "Erstes Admin-Konto eingeladen", ok: m.admins > 0, hinweis: m.admins > 0 ? `${m.admins} Admin-Konto/-Konten aktiv` : "Mandant oben wählen, dann unter „Benutzer“ ein Konto mit Rolle ADMIN einladen.", href: "/benutzer" },
