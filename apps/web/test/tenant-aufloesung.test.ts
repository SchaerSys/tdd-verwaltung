...
describe("Mandanten-Aufloesung aus Rohdaten der Anfrage", () => {
  ...
  it("Umgebung TENANT_HOSTS: nur gueltige Paare", () => {
    expect(hostsAusUmgebung(`tirol.careos.at=${TIROL}; kaputt=abc ; Salzburg.CareOS.at = ${SALZBURG}`)).toEqual({ "tirol.careos.at": TIROL, "salzburg.careos.at": SALZBURG });
  });

  it("Reihenfolge: Session vor Geraet vor Host vor Standard", () => {
    const hosts = { "tirol.careos.at": TIROL };
    const session = signSession({ uid: "u", role: "ADMIN", tenantId: SALZBURG });
    // Session schlaegt Host
    expect(tenantAusRohdaten({ host: "tirol.careos.at", cookie: `tdd_session=${session}` }, hosts)).toBe(SALZBURG);
    // Geraet schlaegt Host
    expect(tenantAusRohdaten({ host: "tirol.careos.at", cookie: `tdd_geraet=${SALZBURG}:token123` }, hosts)).toBe(SALZBURG);
    expect(tenantAusRohdaten({ host: "tirol.careos.at:3080", cookie: `tdd_station=${SALZBURG}:abc` }, hosts)).toBe(SALZBURG);
    // Host mit Port
  ...
  it("gefaelschte Session zaehlt nicht (Signatur), Geraete-Cookie ohne Praefix auch nicht", () => {
    ...
    expect(tenantAusRohdaten({ host: "tirol.careos.at", cookie: `tdd_session=${payload}.falsch` }, hosts)).toBe(TIROL);
    expect(tenantAusRohdaten({ host: "tirol.careos.at", cookie: "tdd_geraet=nurtoken" }, hosts)).toBe(TIROL);
    expect(tenantAusRohdaten({ host: "tirol.careos.at", cookie: "tdd_geraet=nicht-uuid:token" }, hosts)).toBe(TIROL);
  });
});
