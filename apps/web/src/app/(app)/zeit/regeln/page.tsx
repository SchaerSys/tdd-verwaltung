        ...
        <div className="panel">
          ...
          <form action={regelnSpeichern} className="p-4 grid gap-3 sm:grid-cols-2">
            <fieldset disabled={!admin} className="contents">
              ...
              <div className="field"><label className="lbl">Ortung: Ereignisse aufbewahren (Tage)</label><input name="ortungAufbewahrungTage" className="inp mono" defaultValue={r.ortungAufbewahrungTage} /><div className="text-[.7rem] text-muted">Ankunft/Abfahrt aus dem Geofencing; danach automatisch gelöscht (Standard 90)</div></div>
              <div className="sm:col-span-2 border-t border-[color:var(--border)] pt-3 text-[.8125rem] font-semibold">Berichte</div>
              <div className="field sm:col-span-2"><label className="lbl">Monatsbericht per E-Mail an (Obmann/Vorstand, mehrere mit Komma)</label><input name="monatsberichtEmail" className="inp" defaultValue={r.monatsberichtEmail ?? ""} placeholder="obmann@verein.at" /><div className="text-[.7rem] text-muted">Wird am 1. jedes Monats für den Vormonat gesendet (Ausgabe je Ausgabestelle, Klient:innen, Wareneingang, Schulden). Leer = kein Versand.</div></div>
              <div className="sm:col-span-2 border-t border-[color:var(--border)] pt-3 text-[.8125rem] font-semibold">Ausgabe – Schuldengrenzen am Tresen</div>
              <div className="field"><label className="lbl">Warnung ab Schulden (€)</label><input name="schuldenWarnungEur" className="inp mono" defaultValue={r.schuldenWarnungEur} /><div className="text-[.7rem] text-muted">Roter Hinweis am Tresen</div></div>
              <div className="field"><label className="lbl">Sperre ab Schulden (€)</label><input name="schuldenSperreEur" className="inp mono" defaultValue={r.schuldenSperreEur} /><div className="text-[.7rem] text-muted">Ausgabe nur gegen Begleichung des Totals</div></div>
              <div className="field"><label className="lbl">Warnung ab unbezahlten Ausgaben (Anzahl)</label><input name="schuldenWarnungAnzahl" className="inp mono" defaultValue={r.schuldenWarnungAnzahl} /></div>
              <div className="field"><label className="lbl">Sperre ab unbezahlten Ausgaben (Anzahl)</label><input name="schuldenSperreAnzahl" className="inp mono" defaultValue={r.schuldenSperreAnzahl} /><div className="text-[.7rem] text-muted">Es gilt die zuerst erreichte Grenze (Betrag oder Anzahl)</div></div>
              <div className="sm:col-span-2 border-t border-[color:var(--border)] pt-3 text-[.8125rem] font-semibold">Zivildienst – Grenzen laut Zivildienstserviceagentur (§ 23 ZDG)</div>
              <div className="field"><label className="lbl">Wochendienstzeit mindestens (h)</label><input name="ziviWocheMinStd" className="inp mono" defaultValue={r.ziviWocheMinMin / 60} /><div className="text-[.7rem] text-muted">ZISA-Vorgabe, Standard 36 h</div></div>
              <div className="field"><label className="lbl">Wochendienstzeit höchstens (h)</label><input name="ziviWocheMaxStd" className="inp mono" defaultValue={r.zivi.maxWocheMin / 60} /><div className="text-[.7rem] text-muted">Standard 45 h</div></div>
              <div className="field"><label className="lbl">Tageshöchstdienstzeit (h)</label><input name="ziviTagMaxStd" className="inp mono" defaultValue={r.zivi.maxTagMin / 60} /><div className="text-[.7rem] text-muted">Standard 10 h</div></div>
              <div className="field"><label className="lbl">Ruhezeit (h)</label><input name="ziviRuhezeitStd" className="inp mono" defaultValue={r.zivi.ruhezeitMin / 60} /><div className="text-[.7rem] text-muted">Standard 11 h</div></div>
              <div className="field"><label className="lbl">Pause ab (h) / Dauer (min)</label><div className="flex gap-1"><input name="ziviPauseAbStd" className="inp mono" defaultValue={r.zivi.pauseAbMin / 60} /><input name="ziviPauseMin" className="inp mono" defaultValue={r.zivi.pauseMin} /></div></div>
              <div className="field"><label className="lbl">Dienstfreistellung je vollem Monat (Werktage)</label><input name="ziviFreistellungMonat" className="inp mono" defaultValue={r.ziviFreistellungMonat} /><div className="text-[.7rem] text-muted">§ 23a ZDG: 2</div></div>
              <div className="field sm:col-span-2"><label className="flex items-center gap-2 text-[.8125rem]"><input type="checkbox" name="ziviSonntagErlaubt" defaultChecked={r.zivi.sonntagErlaubt} /> Sonn-/Feiertagsdienst für Zivildiener vorgesehen (mit Ersatzruhe) – sonst gilt er als Fehler</label></div>
